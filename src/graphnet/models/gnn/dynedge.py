...
class DynEdge_State_Graph_Illustration(GNN):
    def __init__(self, nb_inputs, save_path, layer_size_scale=4):
        """DynEdge model.
    ...
    def forward(self, data: Data) -> Tensor:
        ...
        if self._save_graphs:
            torch.save(x, self._save_path + "/input_x.pt")
            torch.save(edge_index, self._save_path + "/input_edges.pt")
        ...
        a, edge_index = self.conv_add1(x, edge_index, batch)
        if self._save_graphs:
            torch.save(a, self._save_path + "/state_1_x.pt")
            torch.save(edge_index, self._save_path + "/state_1_edges.pt")
        b, edge_index = self.conv_add2(a, edge_index, batch)
        if self._save_graphs:
            torch.save(b, self._save_path + "/state_2_x.pt")
            torch.save(edge_index, self._save_path + "/state_2_edges.pt")
        c, edge_index = self.conv_add3(b, edge_index, batch)
        if self._save_graphs:
            torch.save(c, self._save_path + "/state_3_x.pt")
            torch.save(edge_index, self._save_path + "/state_3_edges.pt")
        d, edge_index = self.conv_add4(c, edge_index, batch)
        if self._save_graphs:
            torch.save(d, self._save_path + "/state_4_x.pt")
            torch.save(edge_index, self._save_path + "/state_4_edges.pt")
