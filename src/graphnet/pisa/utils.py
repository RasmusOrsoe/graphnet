...
def plot_2D_contour(contour_data, xlim = (0.4, 0.6), ylim = (2.38*1e-3, 2.55*1e-3) ,chi2_critical_value = 4.605, width = 3.176, height = 2.388):
    ...
    for entry in contour_data:
        ...
        for i in range(len(x)):
            for k in range(len(y)):
                idx = (model_data['theta23_fixed'] == x[i]) & (model_data['dm31_fixed'] == y[k])
                match = model_data['mod_chi2'][idx]
                if len(match)>0:
                    if model_data['converged'][idx].values == True:
                        match = float(match)
                    else:
...
def plot_1D_contour(contour_data, chi2_critical_value = 2.706, width = 2*3.176, height = 2.388):
    ...
    fig, ax = plt.subplots(1,2, figsize = (width,height), constrained_layout = True)
    ls = 0
    for entry in contour_data:
        path = entry['path']
        model = entry['model']
        ...
        for variable in variables:
            ...
            model_data = contour_data.loc[(model_idx) & (padding_idx) & (chi2_idx),:]
            x = model_data.sort_values(variable)
            if variable == 'theta23_fixed':
                ax[0].set_ylabel('$\\chi^2$', fontsize = 12)
                ax[0].plot(np.sin(np.deg2rad(x[variable]))**2, x['mod_chi2'], color = color, label = label, ls = ls)
                ax[0].set_xlabel('$\\sin(\\theta_{23})^2$', fontsize = 12)
            elif variable == 'dm31_fixed':
                ax[1].plot(x[variable], x['mod_chi2'], color = color, label = label, ls = ls)
